...
export function SigninForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        const msg = "Server error: Invalid response";
        setError(msg);
        setAlertMessage(msg);
        setShowAlert(true);
        setIsLoading(false);
      ...
      if (response.ok && data.success) {
        ...
        } else if (ADMIN_ROLE_IDS.includes(roleId)) {
          router.replace(`/admin/${roleId}`);
        } else {
          router.replace("/home");
        }
      } else {
      ...
      const msg = "Network error. Periksa koneksi dan coba lagi.";
      setError(msg);
      setAlertMessage(msg);
      setShowAlert(true);
    } finally {
  ...
  return (
    <div
      className={cn("flex flex-col gap-6 bg-black rounded-lg", className)}
      {...props}
    >
      <Card className="w-full h-full bg-black border border-white/30 shadow-2xl">
        <CardHeader className="text-center bg-black pb-6">
          ...
          <CardDescription
            className={`${inter.className} text-sm text-white whitespace-nowrap`}
          >
        ...
        <CardContent>
          {showAlert && (
            <div className="mb-6">
              ...
              >
                <Alert.Indicator
                  className={error ? "text-red-400" : "text-blue-400"}
                />
          ...
          <form onSubmit={handleSubmit}>
            <FieldGroup>
              ...
              <Field>
                <FieldLabel
                  className={`${inter.className} text-white flex justify-between items-center`}
                  htmlFor="password"
                >
                  Password
                  <Link
              ...
              <Field>
                ...
                <div className="relative my-6">
                  ...
                  <div className="relative flex justify-center text-xs">
                    <span
                      className={`${inter.className} bg-black px-3 text-white`}
                    >
                      OR CONTINUE WITH
                    </span>
                  </div>
                ...
                <FieldDescription
                  className={`${inter.className} text-white text-center py-4`}
                >
